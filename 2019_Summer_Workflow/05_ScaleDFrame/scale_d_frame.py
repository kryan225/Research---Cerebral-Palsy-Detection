        ...
        for r_id in recording_ids:
            ...
            try:
                ...
                for row in cursor.fetchall():
                    img = None
                    current = cache_path+str(row[0])+'.d.png'
        ...
        for r_id in recording_ids:
            ...
            try:
                ...
                for row in cursor.fetchall():
                    img = None
                    current_ID = row[0]
                    current_input = cache_path+str(current_ID)+'.d.png'
...
def upload_scaled_images(conn,cache_path):
    cursor = conn.cursor()
    try:
...
def main():
    ...
    print("Attempting to create Scaled Depth Images")
    ...
    try:
        ...
        upload_scaled_images(conn,"cache/")
    finally:
        conn.close()
