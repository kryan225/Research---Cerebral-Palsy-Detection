...
def sql_send(directory, conn=connect()):
    ...
    null_update_ids = []
    check_query = "SELECT raw_id FROM Video_Generated WHERE D_Processed_Updated IS NULL order by raw_id"
    try:
        ...
        while (capture):
            try:
                ...
                if (len(results)>2):
                    ...
                    allTimeQuery = 'SELECT timestamp FROM nicu.Video_Raw WHERE (recording_id = %s) AND (timestamp > %s) ORDER BY timestamp LIMIT 1'
